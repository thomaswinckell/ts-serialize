...
export const defaultUnmarshaller: Unmarshaller< any > = (value: JsValue, json: Json, clazz: any, classPropertyName: string, jsonPropertyName: string, target: Function, mbType: Optional< Function >, jsonPath: string[], classPath: string[]) => {
    ...
    if (!isDefined(value)) {
        ...
        if(isDefined(clazz[classPropertyName])) {
            ...
            const additionalErrorMessage = Some("The value is not defined.");
            return Left<UnmarshallError[], any>([new UnmarshallError(value, mbType, jsonPropertyName, classPropertyName, target, jsonPath, classPath, additionalErrorMessage)]);
        }
    }
...
const stringUnmarshaller: Unmarshaller< string > = (value: JsValue, json: Json, clazz: any, classPropertyName: string, jsonPropertyName: string, target: Function, mbType: Optional< Function >, jsonPath: string[], classPath: string[]) => {
    ...
    const additionalErrorMessage = Some("The value is not a string value.");
    return Left< UnmarshallError[], string >([new UnmarshallError(value, Some(String), jsonPropertyName, classPropertyName, target, jsonPath, classPath, additionalErrorMessage)]);
};
